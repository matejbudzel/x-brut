...
class StorageTest(unittest.TestCase):
    ...
    def test_mount_uses_shared_x4_spi_and_deselects_display(self):
        spi, epd_cs, sd_cs = SPI(), Pin(), Pin()
        board = ModuleType("board")
        board.EPD_CS, board.SD_CS = epd_cs, sd_cs
        board.spi_calls = 0
        ...
        def digital_output(pin):
            result = DigitalInOut(pin)
            outputs.append(result)
            return result

        digitalio.DigitalInOut = digital_output
        displayio = ModuleType("displayio")
        displayio.release_count = 0

        def release_displays():
            displayio.release_count += 1
        ...
        modules = {
            "board": board, "digitalio": digitalio, "displayio": displayio,
            "sdcardio": sdcardio, "storage": circuit_storage,
        }
        ...
        self.assertEqual(displayio.release_count, 1)
        self.assertEqual(board.spi_calls, 1)
        self.assertEqual(len(outputs), 1)
        self.assertIs(outputs[0].pin, epd_cs)
        self.assertIs(outputs[0].value, True)
        self.assertTrue(outputs[0].deinitialized)
