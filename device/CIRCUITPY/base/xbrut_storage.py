...
def mount_sd():
    """Mount the X4 microSD and retain the shared SPI objects for display use."""
    global _spi, _sd, _vfs
    if _spi is not None: return
    import board, digitalio, displayio, sdcardio, storage
    displayio.release_displays()
    try: os.mkdir(DATA_ROOT)
    except OSError: pass
    display_cs = digitalio.DigitalInOut(board.EPD_CS)
    display_cs.switch_to_output(value=True)
    try:
        _spi = board.SPI()
